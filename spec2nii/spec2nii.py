...
class spec2nii:
    def __init__(self):
        ...
        parser_twix.add_argument("-m", "--multiraid", type=int,
                                 help="Select multiraid file to load (default = 2 i.e. 2nd file)", default=2)
        ...
        parser_txt.add_argument("-i", "--imagingfreq", type=float, help="Imaging (central) frequency in MHz",
                                required=True)
        parser_txt.add_argument("-b", "--bandwidth", type=float, help="Reciever bandwidth (sweepwidth) in Hz.",
                                required=True)
        ...
        parser_bruker.add_argument('file', help='file to convert', type=str)
        ...
        parser_bruker.add_argument("-f", "--fileout", type=str, help="Output file base name (default = input file name)")
    ...
    def twix(self, args):
        ...
        imageOrientationPatient, imagePositionPatient, pixelSpacing, slicethickness = twix2DCMOrientation(
            twixObj['hdr'], verbose=args.verbose)
        ...
        if dims[0] != 'Col':
            raise ValueError(
                'Col is expected to be the first dimension in the Twix file, it is not.')  # This is very unlikely to occur  but would cause complete failure.
        if 'Cha' in dims:
            if dims[1] != 'Cha':
                raise ValueError(
                    'If present Cha is expected to be the first dimension in the Twix file, it is not.')  # This is very unlikely to occur but would cause complete failure.
    ...
    def bruker(self, args):
        ...
        for data, properties in read_bruker(args):
            print('Converting dataset id: {}'.format(properties['id']))
            currNiftiOrientation = NIFTIOrient(np.array(properties['affine']))
            self.imageOut.append(data)
            self.orientationInfoOut.append(currNiftiOrientation)
            self.dwellTimes.append(properties['dwell_s'])
            self.fileoutNames.append(properties['id'])
